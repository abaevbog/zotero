...
var CollectionTree = class CollectionTree extends LibraryTree {
	...
	static propTypes = {
		...
		topGroupConfig: PropTypes.object, // { groupIDs: Set, headerLabel: string }
	};
	...
	async refresh() {
		try {
			...
			if (this.props.topGroupConfig) {
				...
				for (let groupID of [...this.props.topGroupConfig.groupIDs]) {
					if (groupID == Zotero.Libraries.userLibraryID) {
						newRows.splice(added++, 0, new Zotero.CollectionTreeRow(this, 'library', Zotero.Libraries.userLibrary, 1, true));
						added += await this._expandRow(newRows, added - 1);
					}
					else {
						newRows.splice(added++, 0, new Zotero.CollectionTreeRow(this, 'group', Zotero.Groups.get(groupID), 1, true));
						added += await this._expandRow(newRows, added - 1);
					}
					added += await this._expandRow(newRows, added - 1);
				}
			...
			let libraryInTopGroup = this.props.topGroupConfig?.groupIDs.has(Zotero.Libraries.userLibraryID);
			...
			if (this.props.topGroupConfig) {
				groups = groups.filter(group => !this.props.topGroupConfig.groupIDs.has(group.id));
			}
